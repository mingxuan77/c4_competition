...
PRESET_PIPELINES = {
    ...
    "产线快速调整": {
        ...
        "tasks": [
            {"task_id": "A", "type": "production_monitor",
             "description": "读取目标产线当前指标和工艺参数"},
            {"task_id": "B", "type": "production_adjuster",
             "description": "计算最优调整量并写入仿真器执行"},
        ],
        "dependencies": {"B": ["A"]},
    },
    "产线自动调整": {
...
class IntentParser:
    ...
    def _parse_with_rules(self, user_input: str) -> dict:
        ...
        preset_triggers = {
            "产线快速调整": ["调高", "调低", "加大", "减小", "调一下", "帮我",
                           "增加", "减少", "改一下", "动一下",
                           "提高", "降低", "升一", "降一", "补偿",
                           "曝光", "功率", "参数", "剂量", "调大", "调小"],
            "产线状态检测": ["检测", "看看产线", "查一下产线", "产线状态", "运行状态",
                           "有什么问题", "正不正常", "指标怎么样", "啥情况"],
            "产线自动调整": ["分析.*调整", "诊断.*调整", "自动调整优化"],
            "产线综合分析": ["综合", "全面分析", "瓶颈", "报告", "深度分析"],
